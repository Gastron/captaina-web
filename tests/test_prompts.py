...
def test_get_prompt(client, uuid):
    ...
    response = client.get(test_lesson.url_id+"/1")
    assert "Äyskäröinti on kivaa" in response.data.decode('utf-8')
    #redirect to first prompt if attempting others or overview first
    response = client.get(test_lesson.url_id+"/overview", follow_redirects=True)
    assert "Äyskäröinti on kivaa" in response.data.decode('utf-8')
    response = client.get(test_lesson.url_id+"/2", follow_redirects=True)
    assert "Äyskäröinti on kivaa" in response.data.decode('utf-8')
